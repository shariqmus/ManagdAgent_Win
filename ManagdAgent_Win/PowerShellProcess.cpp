...
wstring PowerShellProcess::ExecCmd(wstring pCmdArg)
...
//
{
	...
	wstring strResult; // Contains result of cmdArg.
...
wstring PowerShellProcess::ExecPowerShellScript(wstring filePath)
{
	wstring commandLine = L"powershell.exe -NoExit -ExecutionPolicy Unrestricted -File ";

	commandLine += L"\"";
	commandLine += Utility::getCWD();
	commandLine += L"\\";
	commandLine += filePath;
	commandLine += L"\"";

	return CleanResult(ExecCmd(commandLine));
}

wstring PowerShellProcess::CleanResult(wstring subject)
{
	wstring result = L"";
	wistringstream iss(subject);
	...
	for (wstring line; std::getline(iss, line); )
	{
		...
		else if (regex_search(line, sm, wregex(L"^Windows")))
		{
			// Match found
		}
		else if (regex_search(line, sm, wregex(L"^Copyright")))
		{
			// Match found
		}
		else
